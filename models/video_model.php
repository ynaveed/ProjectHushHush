...
class Video_Model extends CI_Model {
	...
	public function save_count_watched_video($video_id) {
		return $video_id;
	}
	public function save_recently_watched($video_id,$user_id){
		$date_time =  date('y-m-j H:i:s');
		$query = $this->db->query('Insert into recently_watched set user_id = "'.$user_id.'", video_id="'.$video_id.'",time_stamp ="'.$date_time.'"');
		return true;
	}
	public function save_favourites($video_id,$user_id){

		$check_query = $this->db->query('select * from favourites where user_id ="'.$user_id.'" and video_id = "'.$video_id.'"');
		if($check_query->num_rows() == 0){
			$date_time =  date('y-m-j H:i:s');
			$query = $this->db->query('Insert into favourites set user_id = "'.$user_id.'", video_id="'.$video_id.'",time_stamp ="'.$date_time.'"');
			return true;
		}else{
			return false;
		}
	...
	public function get_favourites(){
		...
		$query = $this->db->query("select DISTINCT(video_id) from favourites where user_id = {$user_id} limit 10");
		if($query){
			$comma_separated_list = '';
			foreach($query->result() as $row){
				$comma_separated_list .= $row->video_id.',';
			}
			$comma_separated_list = substr($comma_separated_list,0,-1);
			if($comma_separated_list != ''){
				return file_get_contents(API_BASE.'services/get_video?video_id_list='.$comma_separated_list);
			}else{
				return 0;
			}
		}else{
	...
	public function get_all_channels(){
		return file_get_contents(API_BASE.'get_all_channels');
	}
	public function get_drama_by_id($drama_id){
		return file_get_contents(API_BASE.'get_drama?drama_id='.$drama_id);
	}
	public function get_channel_dramas($channel_id){
		return file_get_contents(API_BASE.'get_channel_dramas?channel_id='.$channel_id);
	}
	public function get_all_dramas(){
		$drama_list = '[';
		$temp= $this->get_all_channels();
		$all_channels = json_decode($temp);
		foreach($all_channels as $channel){
			$channel_dramas = $this->get_channel_dramas($channel->id);
			$drama_list .= $this->find_between($channel_dramas,"[","]",false);
			$drama_list .= ',';
			/*foreach($channel->drama_ids as $dramas){
				$drama_list .= $this->get_drama_by_id($dramas);
				$drama_list .= ',';
			}*/
		}
		$final_list = substr($drama_list,0,-1);
		$final_list .= ']';
		return $final_list;
	}
	public function populate_week(){
		$week_dates = "";
		for($i=38;$i<45;$i++){
		...
		$all_released = file_get_contents(API_BASE.'get_released?dates=['.$week_dates.']');
		$new_arr = json_decode($all_released);
		$new_released = array();
		foreach($new_arr as $value){
			foreach($value as $anothervalue){
				$new_released[] = $anothervalue;
			}
		}
		return json_encode($new_released);
	}
	public function weekly_dates(){
		$day_dates = array();
		for($i=38;$i<45;$i++){
			$week_dates = date("Y-m-d", strtotime("-".$i." day"));
			$day = strtolower(date("D", strtotime("-".$i." day")));
			$day_dates[$week_dates] = $day;
		}
		return json_encode($day_dates);
	}
	public function day_at_server(){
		//return strtolower(date("D"));
		return "fri";
	}
	public function ordered_dates(){
		$day_dates = array();
		$count = 7;
		for($i=38;$i<45;$i++){
			$week_dates = date("Y-m-d", strtotime("-".$i." day"));
			$day_dates[$count] = $week_dates;
			$count --;
		}
		return json_encode($day_dates);
	}
	public function get_drama_episodes($drama_id){
		return file_get_contents(API_BASE.'get_drama_episodes?drama_id='.$drama_id);
	}
	public function get_video_player($videoplayer_url){
		return file_get_contents($videoplayer_url);
	}

	public function get_recently_watched(){
		$user_id = $this->session->userdata('user_id');
		$query = $this->db->query("select DISTINCT(video_id) from recently_watched where user_id = {$user_id} limit 10");
		if($query){
			$comma_separated_list = '';
			foreach($query->result() as $row){
				$comma_separated_list .= $row->video_id.',';
	...
	public function get_search_keywords($keywords){
		$keywords = trim(preg_replace('!\s+!', ' ', $keywords));
		$array_of_keywords = explode(' ',$keywords);
		$url_list = '';
		...
		$url_list = substr($url_list,0,-1);
		if($url_list != ''){
			return file_get_contents(API_BASE.'drama_search?keywords='.$url_list.'&type=simple');
		}else{
			return 0;
		}
	...
	public function get_search_results($keywords){
		...
		$url_list = substr($url_list,0,-1);
		if($url_list != ''){
			return file_get_contents(API_BASE.'drama_search?keywords='.$url_list);
		}else{
			return 0;
		}
	}
	private function find_between($s, $str1, $str2, $ignore_case = false) 
	{
